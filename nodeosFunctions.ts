import  { getActionProof, getBaseActionDigest, getDataDigest } from "./ibcFunctions";
...
const getNodeoseHeavyProof = (req, transactions): Promise<Proof>  => new Promise(async (resolve) => {
  ...
  try{
    ...
    const on_block = async function (block){
      if (stopFlag) return;
      let add = true;
...
const formatBlockRes = res => ({
  ...
  header: {
    ...
    transaction_mroot: res.data.transaction_mroot,
    action_mroot: res.data.action_mroot,
    schedule_version: res.data.schedule_version,
    new_producers: res.data.new_producers,
    header_extensions: res.data.header_extensions || []
  },
